 ...
 *
 * @module lib/notifications/__tests__/ride-notify.test
 */
...
import { notifyRide } from '../ride-notify';
...
describe('notifyRide', () => {
  ...
  describe('when it must stay silent', () => {
    it('no-ops instead of throwing when the browser has no Notification API', async () => {
      ...
      await expect(notifyRide(LEAVE_NOTICE)).resolves.toBe(false);
  ...
  describe('when it posts', () => {
    it('shows the notice through the registration and records the fire', async () => {
      ...
      await expect(notifyRide(LEAVE_NOTICE)).resolves.toBe(true);
  ...
  describe('dedupe', () => {
    ...
    it('speaks again about a leg the user has since acknowledged', async () => {
      ...
      await expect(notifyRide(moved)).resolves.toBe(true);
      await expect(notifyRide(moved)).resolves.toBe(false);
