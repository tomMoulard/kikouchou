...
export interface RideCardProps {
  ...
  /**
   * Takes the driver's seat in this car, for the guest this browser is.
   *
   * The counterpart of the same offer on a proposed run: a car can be arranged
   * before anybody has volunteered, and this is how the volunteer arrives —
   * from their own phone, with no dropdown of the whole trip to find
   * themselves in. Absent when this browser is nobody in particular, and the
   * button is then not rendered rather than rendered dead.
   */
  ...
  return (
    ...
    >
      ...
      <CardContent className="pt-0 space-y-2">
        ...
        {/* Who drives, and — while that is nobody — the offer to be them. The
            button sits on this line rather than with the card's actions
            because it answers the sentence beside it. */}
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {!hasDriver && onClaimRide !== undefined && journey.ride !== undefined && !readOnly && (
            <Button
              ...
              disabled={isActionsDisabled || isPast}
            >
