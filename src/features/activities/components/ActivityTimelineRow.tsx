    ...
    () =>
      model.items.map((item) => {
        ...
        return (
          ...
          >
            {timeRange && (
              <span className="shrink-0 text-[10px] font-semibold tabular-nums leading-none opacity-95">
                {timeRange}
              </span>
            )}
            <span className="min-w-0 flex-1 truncate text-left text-[11px] font-medium leading-tight">
              {activity.title}
            </span>
            {participantCount > 0 && (
              <span
