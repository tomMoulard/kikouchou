...
describe('useWebLLM model load failures', () => {
  ...
  it('cancels a load in flight by terminating the worker', async () => {
    ...
    // Transformers.js gives `pipeline()` no abort signal, so killing the worker
    // is the only thing that actually stops the bytes.
    ...
    // A cancellation is not a failure: nothing goes to the load-failure event
    // or to Error tracking.
    expect(
  ...
  it('loads again on a new worker after a cancelled load', async () => {
    ...
    await act(async () => {
      second.reply({ type: 'loaded', requestId: second.requestIdAt(0) });
      await Promise.resolve();
    });
